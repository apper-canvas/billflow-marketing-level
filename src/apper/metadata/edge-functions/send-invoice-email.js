...
apper.serve(async (req) => {
  ...
  try {
    ...
    if (!invoice.client?.email) {
      return new Response(
        JSON.stringify({ success: false, message: 'Client email is required' }),
        { status: 400, headers: { 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!resendApiKey) {
      return new Response(
        JSON.stringify({ 
          success: false, 
          message: 'RESEND_API_KEY secret not configured' 
        }),
      ...
      <html>
        <body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
          <div style="max-width: 600px; margin: 0 auto; padding: 20px;">
            <h2 style="color: #6366f1;">Invoice ${invoice.invoiceNumber}</h2>
            <p>Dear ${invoice.client?.contactPerson || 'Valued Client'},</p>
            <p>Please find attached invoice ${invoice.invoiceNumber} for your review.</p>
    ...
    const result = await resend.emails.send({
      ...
      subject: `Invoice ${invoice.invoiceNumber} - Amount Due: $${invoice.total.toFixed(2)}`,
      html: emailHtml,
      attachments: [
        {
